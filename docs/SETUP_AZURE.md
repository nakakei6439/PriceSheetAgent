...
6. デプロイ設定 (**実績ベースの推奨値**):
   - **デプロイ名**: `gpt-4o` (`.env` のデフォルトと一致させると楽)
   - **モデルバージョン**: `2024-11-20` (deprecating でない最新)
   - **デプロイの種類**: **Standard** ← **新規アカウントでは Global Standard だとクォータ 0 になりがち**。Standard なら East US 2 等に 50K TPM 確保できる
   - **接続されている AI リソース**: 自動作成される子リソース (例: `<ユーザ名>-<ランダム>-eastus2`) — そのままで OK
   - **1分あたりのトークン (TPM)**: **50K** (スライダー右端まで、または可能な最大)
   - **RPM**: 300 程度になればOK
   - **コンテンツフィルター**: DefaultV2 のまま
   - **動的クォータ**: 有効
7. 「**デプロイ**」 → 数秒〜1分で "成功"

> ⚠ **GPT-4o デプロイで詰まったとき** (新規アカウント 1 日目によくある)
> - `ServiceModelDeprecating` (2024-08-06 など古い版) → モデルバージョンを `2024-11-20` などより新しいものに変更
> - `クォータ不足` (Global Standard 選択時) → **Standard** に変更すると割り当てられることがある
> - それでもダメなら **gpt-4o-mini** や **gpt-4.1-mini** で代替 (どちらも Vision 対応)。デプロイ名を `gpt-4o-mini` 等にして `.env` の `AZURE_OPENAI_GPT4O_DEPLOYMENT` を一致させればコード側は無修正で動く
...
#### GPT-4o (実は子リソースから取得する)

`Foundry リソース (mahted-foundry)` ではなく、**8-2 で自動作成された "接続されているAIリソース" (例: `nakak-mpeo8drm-eastus2`) から取得します**。

- 取得場所:
  - Foundry ポータル → 該当デプロイメントの詳細画面 → 「ターゲットURI」と「キー」が並んで表示される、または
  - Azureポータル → リソースグループ `rg-mahted-dev` → `<ユーザ名>-<ランダム>-eastus2` → 左メニュー「**キーとエンドポイント**」
- 表示されるターゲットURIは **完全なチャットURL** (例: `https://nakak-mpeo8drm-eastus2.cognitiveservices.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2025-01-01-preview`)
- **`.env` には "ベースURL" のみを貼る**: `https://nakak-mpeo8drm-eastus2.cognitiveservices.azure.com/`
- API バージョンは `.env` 側で個別指定するので、URL 末尾の `?api-version=...` は **無視**

#### Document Intelligence の値
- `mahted-di` リソース → 左メニュー「**キーとエンドポイント**」
- 控える項目:
  - **エンドポイント**
  - **KEY 1**
...
cp .env.example .env
```
そして `.env` を以下のように埋める (実例):
```
# gpt-4o (2024-11-20) Standard デプロイ @ East US 2
# 接続リソース: <ユーザ名>-<ランダム>-eastus2
AZURE_OPENAI_ENDPOINT=https://<your-connected-resource>.cognitiveservices.azure.com/
AZURE_OPENAI_API_KEY=（ターゲットURIと一緒に表示されたキー or 接続リソース → キーとエンドポイント → KEY 1）
...
⚠ **セキュリティ**: `.env` は `.gitignore` 済みでコミットされませんが、**スクリーンショットや画面共有でキーが第三者に見えると無効**になります。漏れた疑いがあれば該当リソースの「キーとエンドポイント → Key 1 の再生成」でローテートしてください。

⚠ Foundry リソース自体 (`mahted-foundry`) のエンドポイント (`*.openai.azure.com` または `*.services.ai.azure.com`) は **モデルがホストされている子リソースとは別物**。今回のデプロイでは子リソース側を参照します。
...
| エラー | 原因 | 対処 |
|---|---|---|
| `Insufficient quota for gpt-4o GlobalStandard` (新規アカウント 1 日目で頻発) | Global Standard のクォータが 0 で割り当て不可 | **デプロイの種類を Standard に変更** すると別リージョン (East US 2 等) の子リソースが Azure 側で自動作成され、50K TPM が取れることが多い |
| `InvalidCapacity: capacity '0' ... should be at least 1` | TPM スライダーが 0 のままデプロイした | スライダー右端までドラッグ / 数値欄に直接入力 / つまみを選択し → キー連打 |
| `OpenAI: location not available` | GPT-4o の対応リージョンでない | Foundry リソース自体を East US / Sweden Central / West US 3 のいずれかで再作成 |
| `ServiceModelDeprecating: ... gpt-4o ... 2024-08-06 ... cannot be used for new deployments` | ポータルの既定選択が新規デプロイ不可の古い GPT-4o バージョンになっている | デプロイ画面で **モデルバージョン `2024-11-20`** を明示選択する。選べない場合は `gpt-4.1` など利用可能な後継モデルでデプロイし、`.env` の `AZURE_OPENAI_GPT4O_DEPLOYMENT` をそのデプロイ名に合わせる |
...
【やろうとしていること】Microsoft Foundry リソース (East US) で gpt-4o をデプロイし、
                       Document Intelligence F0 と組み合わせて価格通知書・価格表PDFから商品コード/価格を抽出
【発生エラー】
（エラーメッセージをコピペ）
