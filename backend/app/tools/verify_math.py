...
def verify(items: list[LineItem], total: float | None) -> tuple[bool, list[str], TraceStep]:
    started = time.time()
    warnings: list[str] = []
    ...
    if total is not None and items:
        if not _close(summed, total, rel=0.03):
...
def stamp_result(items: list[LineItem], total: float | None, meta, trace_so_far: list[TraceStep]) -> ExtractionResult:
    passed, warnings, step = verify(items, total)
